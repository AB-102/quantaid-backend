...
def submit_feedback():
    ...
    try:
        ...
        return jsonify({"message": "Feedback submitted successfully", "feedback_id": str(result.inserted_id)}), 200
...
def export_feedback_csv():
    ...
    try:
        ...
        feedback_list = list(
            db.feedback.find(
                query, {"user_id": 1, "category": 1, "feedback": 1, "status": 1, "created_at": 1, "screenshot_id": 1}
            ).sort("created_at", -1)
        )
...
def update_feedback_status(feedback_id):
    ...
    try:
        ...
        result = db.feedback.update_one(
            {"_id": ObjectId(feedback_id)}, {"$set": {"status": new_status, "updated_at": datetime.now(UTC)}}
        )
