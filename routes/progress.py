...
def flush_quiz_buffer():
    ...
    # Distributed lock prevents duplicate flushes across Gunicorn workers
    lock = redis_client.lock("flush_quiz_buffer_lock", timeout=25)
    ...
    try:
        ...
        while True:
            ...
            try:
                ...
                logger.warning("Skipping corrupted quiz buffer entry: %s", e)
        ...
        for entry in entries:
            ...
            operations.append(UpdateOne({"user_id": entry["user_id"]}, {"$push": {"completedQuizzes": quiz_doc}}))
...
def save_quiz_result():
    ...
    try:
        ...
        if redis_client:
            ...
            quiz_result_doc = {"courseId": course_id, "score": score, "passed": passed, "timestamp": timestamp}
        ...
        return jsonify({"message": "Quiz result saved successfully", "unlockedLevels": new_unlocked_levels}), 200
...
def save_first_lesson_view():
    ...
    try:
        update_result = db.users.update_one(
            {"user_id": user_id}, {"$set": {"hasViewedFirstLesson": True, "firstLessonViewedAt": datetime.now(UTC)}}
        )
...
def get_user_progress():
    ...
    try:
        user = db.users.find_one(
            {"user_id": user_id},
            {"_id": 0, "unlockedLevels": 1, "completedQuizzes": 1, "hasViewedFirstLesson": 1, "name": 1, "picture": 1},
        )
        if not user:
            logout_user()
            session.clear()
