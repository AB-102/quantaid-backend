...
def append_user_id():
    ...
    try:
        ...
        if email == FIRST_TIME_USER_EMAIL:
            ...
            if user_doc and user_doc.get("disabled", False):
            ...
            if not user_doc:
                ...
                # Rotate login_id and update is_admin for existing test user
                new_login_id = generate_login_id()
                update_fields: dict = {"login_id": new_login_id, "is_admin": is_admin}
                if google_sub and not user_doc.get("google_sub"):
                    update_fields["google_sub"] = google_sub
                db.users.update_one({"user_id": email}, {"$set": update_fields})
            ...
            return jsonify(
                {
                    "message": "Test user - redirecting to onboarding",
        ...
        if existing_user:
            ...
            db.users.update_one({"user_id": existing_user["user_id"]}, {"$set": update_fields})
            ...
            if existing_user.get("profile_complete"):
                return jsonify(
                    {
                        "message": "User exists and profile is complete",
                        "redirect_to": "map",
                        "is_admin": is_admin,
                        "email": email,
                    }
        ...
        update_result = db.users.update_one(
            {"user_id": user_id},
            {
                "$set": {
                    ...
                    "profileLastUpdatedAt": datetime.now(UTC),
...
def save_profile_picture():
    ...
    try:
        ...
        db.users.update_one({"user_id": user_id}, {"$set": {"picture": picture_url}})
