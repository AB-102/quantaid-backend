...
from passlib.hash import argon2  # ty: ignore[unresolved-import]
...
def authenticate_user(email: str, password: str) -> tuple[User | None, str]:
    ...
    if user.is_locked and user.locked_until:
        ...
        remaining = locked - datetime.now(UTC)
...
def _record_failed_attempt(email: str):
    ...
    if result and result.get("failed_login_attempts", 0) >= MAX_FAILED_ATTEMPTS:
        db.users.update_one(
            {"user_id": email.lower()}, {"$set": {"locked_until": datetime.now(UTC) + LOCKOUT_DURATION}}
        )
